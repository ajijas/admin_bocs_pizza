...
class SystemController extends Controller
{
    ...
    {
        // Bypass purchase code validation for development or when bypass is enabled
        if (env('APP_MODE') != 'live' || env('BYPASS_ACTIVATION', false)) {
            ...
            $stored_url = session('stored_url', route('admin.dashboard'));
            session()->forget('stored_url');
            return redirect($stored_url);
        }

        // Original validation logic for live mode
        $post = [
    ...
    /**
     * Activate all apps at once (bypass method)
     * @return RedirectResponse
     */
    public function activateAllApps(): RedirectResponse
    {
        $previous_active = json_decode($this->businessSetting->where('key', 'app_activation')->first()->value ?? '[]');

        // Get all app IDs from APPS constant
        $all_apps = [];
        foreach (APPS as $app) {
            $all_apps[] = [
        ...
        // Merge with existing activations (avoid duplicates)
        $existing_ids = array_column($previous_active, 'software_id');
        foreach ($all_apps as $app) {
            if (!in_array($app['software_id'], $existing_ids)) {
                $previous_active[] = $app;
            }
        }
        ...
        Toastr::success('All apps activated successfully!');
