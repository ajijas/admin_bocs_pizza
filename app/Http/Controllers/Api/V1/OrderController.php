    ...
    public function placeOrder(Request $request): JsonResponse
    {
        ...
        if ($request['order_type'] == 'dine_in') {
            if (!$request->has('table_id') || $request['table_id'] == null) {
                return response()->json(['errors' => [['code' => 'table-required', 'message' => translate('table selection is required for dine-in orders')]]], 403);
            }
            if (!$request->has('number_of_people') || $request['number_of_people'] == null) {
                return response()->json(['errors' => [['code' => 'people-count-required', 'message' => translate('number of people is required for dine-in orders')]]], 403);
            }
        }
       ...
       // $preparation_time = Helpers::get_business_settings('default_preparation_time') ?? 0;
        ...
        try {
            ...
            $or = [
                ...
                'number_of_people' => $request['order_type'] == 'dine_in' ? $request['number_of_people'] : null,
                'created_at' => now(),
                'updated_at' => now()
            ];
            ...
            // Create table order for dine-in orders
            if ($request['order_type'] == 'dine_in' && $request['table_id']) {
                $tableOrder = new \App\Model\TableOrder();
                $tableOrder->order_id = $or['id'];
                $tableOrder->table_id = $request['table_id'];
                $tableOrder->branch_table_token = Helpers::generate_order_id();
